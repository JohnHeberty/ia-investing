...
from connectors.cvm._financials import FinancialEntry
...
def normalize_bpa(rows: list[dict]) -> dict[str, float]:
    ...
    asset_current = {
        "caixa", "aplicacoes_financeiras", "contas_receber", "estoques",
        "ativos_biologicos", "tributos_a_receber", "adiantamentos",
...
def normalize_bpp(rows: list[dict]) -> dict[str, float]:
    ...
    liab_current = {
        "fornecedores", "emprestimos_circulantes", "divida_bancaria_circulante",
        "obrigacoes_fiscais", "obrigacoes_trabalhistas", "dividendos_a_pagar",
        "adiantamentos_clientes", "outros_passivos_circulantes",
    }

    liab_non_current = {
        "emprestimos_nao_circulantes", "divida_bancaria_nao_circulante",
        "debentures", "obrigacoes_afetadas", "provisoes", "passivos_ambientais",
        "outros_passivos_nao_circulantes",
    }

    equity_accounts = {
        "capital_social", "reservas", "lucros_acumulados",
        "resultados_nao_apropriados", "ajustes_avaliacao", "outros",
    }
    ...
    for r in rows:
        if isinstance(r, FinancialEntry):
            ...
            entries.append(FinancialEntry(
                ...
                valor=float(r.get("valor", r.get("VL_CONTA", 0.0)) or 0.0),
