...
class PolicySource(Base):
    ...
    authority: Mapped[str] = mapped_column(sa.String(100), default="camara")
    fetch_config: Mapped[dict[str, object]] = mapped_column(sa.JSON, default=dict)
    last_fetch_error: Mapped[str | None] = mapped_column(sa.Text)
    last_fetch_error_at: Mapped[datetime | None] = mapped_column(sa.DateTime(timezone=True))
    created_at: Mapped[datetime] = mapped_column(sa.DateTime(timezone=True), default=utcnow)
    updated_at: Mapped[datetime] = mapped_column(sa.DateTime(timezone=True), default=utcnow, onupdate=utcnow)
