...
def test_api_has_no_live_order_surface_or_broker_dependency() -> None:
    ...
    dependencies = (Path(__file__).parents[4] / "pyproject.toml").read_text(encoding="utf-8").lower()
    ...
    configuration = "\n".join(
        ...
        for path in (Path(__file__).parents[4] / ".env.example", Path(__file__).parents[4] / "docker-compose.yml")
    ).lower()
