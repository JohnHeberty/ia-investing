...
def test_schedules_start_registered_public_workflows():
    root = Path(__file__).resolve().parents[2]
    schedules = (root / "src/apps/scheduler/temporal_schedules.py").read_text(encoding="utf-8")

    assert "IngestCVMWorkflow.run" in schedules
    assert "PaperValuationWorkflow.run" in schedules
    assert "PaperReconciliationWorkflow.run" in schedules
    assert "NewsDedupWorkflow.run" in schedules
