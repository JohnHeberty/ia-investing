...
export function SourcesTable({
  ...
  sources: NewsSource[];
  stats: SourceStats | null | undefined;
}) {
    ...
    <>
      {stats && (
        <section
          ...
          aria-label="Metricas de fontes"
          aria-live="polite"
        >
          <Metric
            label="Fontes ativas"
            value={String(stats.active_sources)}
            note="RSS configuradas"
          />
          <Metric
            label="Itens coletados"
            value={String(stats.total_items)}
            note={`${stats.unprocessed_items} pendentes`}
          />
          <Metric
            label="Eventos"
            value={String(stats.total_events)}
            note={`${stats.positive_events} pos / ${stats.negative_events} neg`}
          />
