    ...
    <article className={styles.source}>
      <div className={styles.sourceHeader}>
        <div>
          ...
          <div className={styles.meta}>
            quantitativo {score(item.quantitative_score)} · dados {score(item.data_coverage_score)}{" "}
            · fontes {score(item.source_discovery_score)}
          </div>
        </div>
      ...
      {!!item.signals.length && (
        <p className="subtitle">
          <strong>Sinais:</strong> {item.signals.join(" · ")}
        </p>
      )}
      {!!item.risks.length && (
        <p className="subtitle">
          <ShieldAlert size={13} /> <strong>Riscos:</strong> {item.risks.join(" · ")}
        </p>
      )}
      {item.status === "new" && (
        <div className={styles.actions}>
          ...
          >
            <CheckCircle2 size={14} /> Promover para investigação
          </button>
          <button
      ...
      {item.promoted_candidate_id && (
        <Link href={`/opportunities/candidates/${item.promoted_candidate_id}`}>
          Abrir candidato promovido
        </Link>
      )}
    </article>
  );
