...
export function GapCard({
  ...
  return (
    <article
      className={`${styles.gap} ${gap.status === "open" && gap.level === "blocking" ? styles.blocker : ""}`}
    >
      <div className={styles.gapHeader}>
        <strong>{gap.title}</strong>
        <span
          className="badge"
          data-tone={gap.status === "resolved" ? "good" : gap.level === "blocking" ? "bad" : "warn"}
        >
      ...
      <p className="subtitle">
        <strong>Ação:</strong> {gap.requested_user_action}
      </p>
      {gap.status === "open" && editingGapId !== gap.id && (
        <button className="button" style={{ marginTop: 8 }} onClick={() => onEdit(gap.id)}>
          Resolver
        </button>
      )}
      {editingGapId === gap.id && (
        ...
        >
          ...
          <div className={styles.resolveActions}>
            <button
              type="submit"
              className="button"
              disabled={resolveNotes.length < 3 || resolving}
            >
              {resolving ? "Salvando..." : "Salvar"}
            </button>
      ...
      {gap.status === "resolved" && gap.resolution_notes && (
        <div className={styles.resolvedInfo}>
          <p className="subtitle">
            <strong>Resolvido por:</strong> {gap.resolved_by}
          </p>
          <p className="subtitle">
            <strong>Em:</strong>{" "}
            {gap.resolved_at
              ? new Intl.DateTimeFormat("pt-BR", {
                  dateStyle: "medium",
                  timeStyle: "medium",
                }).format(new Date(gap.resolved_at))
              : "—"}
          </p>
          <p className="subtitle">
            <strong>Notas:</strong> {gap.resolution_notes}
          </p>
        </div>
      )}
    </article>
