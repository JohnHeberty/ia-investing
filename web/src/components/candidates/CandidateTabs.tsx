...
const REQUIRED_KINDS: SourceKind[] = ["cvm_profile", "cvm_filings", "b3_listing", "investor_relations", "financial_reports"];

function OverviewPanel({ detail, candidate, sourceByKind }: { detail: CandidateDetail; candidate: CandidateDetail["candidate"]; sourceByKind: Map<SourceKind, CandidateDetail["sources"][number]> }) {
  return (
    <div id="panel-overview" role="tabpanel" aria-labelledby="tab-overview" className={`${styles.layout} tab-panel`}>
      <div className="card-title"><h2>Checklist de fontes bloqueantes</h2><span>liberação determinística</span></div>
      <div className={styles.statusGrid}>{REQUIRED_KINDS.map((kind) => {
...
function SourcesPanel({ detail, candidate, openGaps, etag, load }: { detail: CandidateDetail; candidate: CandidateDetail["candidate"]; openGaps: CandidateDetail["gaps"]; etag: string; load: () => Promise<void> }) {
  return (
    <div id="panel-sources" role="tabpanel" aria-labelledby="tab-sources" className="split">
      <div className={styles.sourceList}>
        <div className="card-title"><h2>Fontes encontradas e fornecidas</h2><span>{detail.sources.length}</span></div>
        {detail.sources.length === 0 ? (
          <div className="state-panel"><strong>Nenhuma fonte registrada</strong>Use o formulário ao lado para complementar.</div>
        ) : detail.sources.map((source) => (
          <article key={source.id} className={styles.source}>
            <div className={styles.sourceHeader}>
              <strong>{sourceLabels[source.kind]}</strong>
              <span className="badge" data-tone={source.status === "verified" ? "good" : source.status === "rejected" ? "bad" : "warn"}>{source.status}</span>
            </div>
            <a href={source.url} target="_blank" rel="noreferrer">{source.url} <ExternalLink size={11} /></a>
            <div className={styles.meta}>{source.verification_method} · confiança {Math.round(Number(source.confidence) * 100)}% · {source.official ? "declarada oficial" : "não confirmada"}</div>
          </article>
        ))}
      </div>
      <aside className="card card-pad">
        <div className="card-title"><h2>Complementar fonte</h2><span>validação obrigatória</span></div>
        <SourceCompletionForm candidateId={candidate.id} etag={etag} suggestedKind={openGaps.find((gap) => gap.source_kind)?.source_kind as SourceKind ?? null} onSaved={() => void load()} />
      </aside>
    </div>
  );
}

function GapsPanel({ detail, editingGapId, resolveNotes, resolving, onResolveGap, onEditGap, onCancelEdit, onNotesChange }: {
  ...
  return (
    <div id="panel-gaps" role="tabpanel" aria-labelledby="tab-gaps" className={`${styles.gapList} tab-panel`}>
      {detail.gaps.map((gap) => (
        <GapCard
          key={gap.id}
          gap={gap}
...
function AnalysisPanel({ runs }: { runs: CandidateDetail["analysis_runs"] }) {
  return (
    <div id="panel-analysis" role="tabpanel" aria-labelledby="tab-analysis" className="table-wrap tab-panel">
      <table className="table">
        <thead><tr><th>Execução</th><th>Gatilho</th><th>Estado</th><th>Decisão</th><th>Data de referência</th><th>Bloqueios</th></tr></thead>
        <tbody>{runs.map((run) => (
          <tr key={run.id}>
            ...
            <td>{run.status}</td>
            <td>{run.decision ?? "—"}</td>
            <td>{new Intl.DateTimeFormat("pt-BR", { dateStyle: "short", timeStyle: "short" }).format(new Date(run.data_as_of))}</td>
...
function TimelinePanel({ events }: { events: CandidateDetail["timeline"] }) {
  return (
    <div id="panel-timeline" role="tabpanel" aria-labelledby="tab-timeline" className={`${styles.timeline} tab-panel`}>
      {events.map((event) => (
        <article key={event.id} className={styles.timelineItem}>
          <strong>{event.event_type}</strong>
          <p className="subtitle">{event.actor_type}: {event.actor_id}</p>
          <div className={styles.meta}>{new Intl.DateTimeFormat("pt-BR", { dateStyle: "medium", timeStyle: "medium" }).format(new Date(event.occurred_at))} · versão {event.aggregate_version}</div>
        </article>
      ))}
    </div>
  );
    ...
    <section className="card card-pad section-gap" aria-live="polite">
      <div className={styles.tabs} role="tablist" aria-label="Detalhes do candidato">
        {(Object.keys(TAB_LABELS) as Tab[]).map((value) => (
          <button key={value} id={`tab-${value}`} className={`${styles.tab} ${tab === value ? styles.tabActive : ""}`} role="tab" aria-selected={tab === value} aria-controls={`panel-${value}`} onClick={() => setTab(value)} onKeyDown={(e) => handleKeyDown(e, value)}>
            {TAB_LABELS[value]}
          </button>
        ))}
      </div>

      {tab === "overview" && <OverviewPanel detail={detail} candidate={candidate} sourceByKind={sourceByKind} />}
      {tab === "sources" && <SourcesPanel detail={detail} candidate={candidate} openGaps={openGaps} etag={etag} load={load} />}
      {tab === "gaps" && <GapsPanel detail={detail} editingGapId={editingGapId} resolveNotes={resolveNotes} resolving={resolving} onResolveGap={onResolveGap} onEditGap={onEditGap} onCancelEdit={onCancelEdit} onNotesChange={onNotesChange} />}
