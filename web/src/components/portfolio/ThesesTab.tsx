...
function recBadgeTone(rec: string): "good" | "warn" | "bad" | "neutral" {
  switch (rec) {
    case "buy": return "good";
    case "sell": return "bad";
    ...
    <div className="card card-pad" style={{ marginBottom: 12 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12, marginTop: 12 }}>
        {thesis.assumptions.length > 0 && (
          <div>
            <div style={{ fontSize: 11, fontWeight: 500, color: "var(--accent)", marginBottom: 4 }}>Premissas</div>
        ...
        {thesis.catalysts.length > 0 && (
          <div>
            <div style={{ fontSize: 11, fontWeight: 500, color: "var(--blue)", marginBottom: 4 }}>Catalisadores</div>
        ...
        {thesis.risks.length > 0 && (
          <div>
            <div style={{ fontSize: 11, fontWeight: 500, color: "var(--amber)", marginBottom: 4 }}>Riscos</div>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", marginTop: 12, paddingTop: 8, borderTop: "1px solid var(--line)", fontSize: 11, color: "var(--muted)" }}>
        <span>
        ...
        <span>
          ...
          {" · "}
          Expira:{" "}
          <span style={{ color: isExpired ? "var(--red)" : isExpiringSoon ? "var(--amber)" : "var(--muted)" }}>
  ...
  if (isLoading) {
    return (
      <StatePanel
        ...
        detail="Carregando teses vinculadas..."
      />
    );
  }
  ...
  return (
    <div>
      <div className="card card-pad" style={{ marginBottom: 16 }}>
        <div className="card-title">
          <h2>Teses de Investimento</h2>
          <Badge tone="neutral">{theses.length} tese{theses.length !== 1 ? "s" : ""}</Badge>
        </div>
